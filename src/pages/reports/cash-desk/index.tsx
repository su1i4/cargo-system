...
export const CashDeskReport = () => {
  const setTitle = useDocumentTitle();

  useEffect(() => {
    setTitle("Отчет по приходам и расходам");
  }, []);
  ...
  const buildQueryParams = () => {
    ...
    if (operationType !== "all") {
      filters.push({ type: { $eq: operationType } });
    }
  ...
  const downloadXLSX = async () => {
    try {
      ...
      const fileName = `отчет_приходы_расходы_${dayjs().format(
        "DD-MM-YYYY_HH-mm"
      )}.xlsx`;
  ...
  const downloadCSV = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `отчет_приходы_расходы_${dayjs().format("DD-MM-YYYY_HH-mm")}.csv`
      );
      ...
      <Row gutter={[16, 16]} align="middle" style={{ marginBottom: 16 }}>
        ...
        <Col>
          ...
          >
            ...
            <Select.Option value="income">Приходы</Select.Option>
            <Select.Option value="outcome">Расходы</Select.Option>
          </Select>
        </Col>
        <Col flex="auto">
          <Input
