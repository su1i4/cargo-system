...
// Маппинг типов операций для приходов
...
export const CashDeskIncomeReport = () => {
  const setTitle = useDocumentTitle();

  useEffect(() => {
    setTitle("Отчет по приходам");
  }, []);

  const [sortDirection, setSortDirection] = useState<"ASC" | "DESC">("DESC");
  const [sortField, setSortField] = useState<"id" | "date" | "amount">("date");
  const [searchFilters, setSearchFilters] = useState<any[]>([
    { type: { $eq: "income" } }, // Только приходы
  ]);
  ...
  const setFilters = (
    ...
    if (mode === "replace") {
      // Всегда сохраняем фильтр на тип income
      setSearchFilters([{ type: { $eq: "income" } }, ...filters]);
    } else {
  ...
  const prepareExportData = () => {
    ...
    return dataSource.map((record: any, index: number) => ({
      ...
      "Дата прихода": record.date
        ...
        : "",
      "Вид прихода": typeOperationMap[record.type_operation] || record.type_operation || "",
  ...
  const downloadXLSX = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "Отчет по приходам");

      const fileName = `отчет_приходы_${dayjs().format(
        "DD-MM-YYYY_HH-mm"
      )}.xlsx`;
  ...
  const downloadCSV = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `отчет_приходы_${dayjs().format("DD-MM-YYYY_HH-mm")}.csv`
      );
